...
/**
  ...
  * @param `type`           Indicates the type of transportation used on a route. Valid options are:
  ...
  * @param continuousPickup  Indicates whether a rider can board the transit vehicle anywhere along the vehicle’s travel path. The path is described by shapes.txt on every trip of the route. Valid options are:
